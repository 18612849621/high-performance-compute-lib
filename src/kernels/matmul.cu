...
__device__ void DisplayThreadInfo(int eid, int bid, int tid, int col_num,
                                  float *A, float *B) {
  printf("[execute_%d, block_%d, thread_%d]: %f %f %f %f\n", eid, bid, tid,
         A[eid * col_num], B[0], A[eid * col_num + 1], B[1]);
}
...
__global__ void Matmul(float *A, float *B, float *C) {
  ...
  C[execute_id] =
      A[col_num * execute_id] * B[0] + A[col_num * execute_id + 1] * B[1];
}
...
void MatmulTest() {
  ...
  Matmul<<<block_num, thread_num>>>(d_A, d_B, d_C);
  // 错误检查
  ...
  // device -> host
  cudaMemcpy(h_C, d_C, 2 * sizeof(float),
             cudaMemcpyDeviceToHost); // display result
  for (int i = 0; i < 2; ++i) {
    printf("%f \n", h_C[i]);
  }
  // free
